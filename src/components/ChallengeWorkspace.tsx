...
export function ChallengeWorkspace({
  ...
  useEffect(() => {
    ...
    setIsCompletedLocally(window.localStorage.getItem(`${storageKey}:completed`) === "true");
  }, [storageKey]);
  ...
  function completeLesson() {
    startTransition(async () => {
      ...
      setMessage(
        result.ok
          ? result.message
          : "Кейс засчитан локально. После подключения базы прогресс будет сохраняться в аккаунт."
      );
    });
  }
      ...
      <div className="border-b border-line bg-brand/10 px-5 py-4">
        ...
        {isCompletedLocally ? (
          <p className="mt-2 text-sm text-green-100">
            Кейс уже закрыт в этом браузере.
          </p>
        ) : null}
      </div>
            ...
            <div className="flex items-center justify-between border-b border-line px-4 py-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  window.localStorage.removeItem(`${storageKey}:completed`);
                }}
                type="button"
              >
                  Сброс
                ...
                <button
                  ...
                  type="button"
                >
                  {isCompletedLocally ? "Завершено" : "Завершить"}
                </button>
              </div>
            </div>
            <iframe
